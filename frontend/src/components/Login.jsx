...
function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (err.response && err.response.status === 404) setError("User does not exist!"); 
      
      // LINE 19: If server says "401", the password does not match
      else if (err.response && err.response.status === 401) setError("Wrong password!"); 
  ...
  return (
    ...
    <div className="flex-1 flex items-center justify-center min-h-screen p-4 bg-gray-100">
      <div className="bg-white p-8 rounded-xl shadow-lg w-96 border mx-auto">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button className="w-full bg-blue-500 text-white p-3 rounded-lg font-bold hover:bg-blue-600 transition-colors">
            Login
          </button>
        ...
        <button 
          ...
          className="mt-4 text-blue-500 w-full text-sm font-medium hover:text-blue-600"
        >
